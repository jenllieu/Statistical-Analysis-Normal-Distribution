---
title: "Lab 1"
author: "Jennifer Lieu UNI: jll2234"
...
mean100 <- mean(normal100)
sd100 <- sd(normal100)
```

The mean and standard deviation is not exactly 0 and 1 because we are generating a sample from the distribution. We are not looking at the distribution itself. 
...
```{r}
normal10 <- rnorm(n = 10)
mean10 <- mean(normal10)
sd10 <- sd(normal10)

normal1000 <- rnorm(n = 1000)
mean1000 <- mean(normal1000)
sd1000 <- sd(normal1000)

normal10000 <- rnorm(n = 10000)
mean10000 <- mean(normal10000)
sd10000 <- sd(normal10000)

normal100000 <- rnorm(n = 100000)
mean100000 <- mean(normal100000)
sd100000 <- sd(normal100000)
```
...
```{r}
sample_means <- c(mean10, mean100, mean1000, mean10000, mean100000)
lengthmeans <- length(sample_means)
```
As the sample size increases, the mean approaches 0, the true mean of the distribution of X.
...
mean1mil <- mean(normal1mil)
sd1mil <- sd(normal1mil)
```
...
```{r}
normalgreater3 <- normal1mil[normal1mil>3]
```
...
```{r}
mean1234 <- mean(normal1mil_mat[,1234])
```
...
```{r}
colSums1mil <- colSums(normal1mil_mat)/100
```
...
hist(colSums1mil)
```
This sample mean is normally distributed.
